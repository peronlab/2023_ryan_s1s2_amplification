...
%    plot_proj_single_set(all_dat, vali, all_dat.s1_id_range, s2_to_s1_abs_ax, 's2p neurons in s1', 5);
    ...
    vali = find(all_dat.retrograde_label_type ==  2); % 0 = none 1 = retrograde injected into s1 2 = retrograde injected into s2
    plot_proj_single_set(all_dat, vali, s1_tag, s2_to_s1_mean_resp_proj_ax, 's2p neurons in s1', 7, 'mean response to touch, proj');
...
function plot_proj_single_set(all_dat, vali, restrict_tag, ax, tstr, mode, ylab);
    ...
    for v=1:length(vali)
        ...
        N_red = length(intersect(all_dat.types_by_idx{ai}.is_red,area_nrni));
        ...
        %N_red_nontouch(v)= N_red_nontouch;
        N_all_anims(v)= N_all; 
        N_red_anims(v)= N_red; 
...
%        mu_resp_per_cell = resp_dff_mat;    
    ...
    disp(sprintf('%s num cells %f SD %f num red %f SD %f' , tstr, mean(N_all_anims), std(N_all_anims),mean(N_red_anims),std(N_red_anims)));
    ms = 10;
...
%         if mode==4 
%             %A= ratio_mat(~isnan(ratio_mat(1,:))); B= ratio_mat(~isnan(ratio_mat(2,:))); C= ratio_mat(~isnan(ratio_mat(3,:)));
%             ratio_mat_test= ratio_mat';
%             anova_test= anova1(ratio_mat_test);
%             disp(sprintf('%s %s anova ', ylab, tstr,anova_test));
%         else 
...
%         if mode==2 
%             
%             [h1,pv1]= ttest((ratio_mat(1,:)),1);
%             [h2, pv2]= ttest((ratio_mat(2,:)),1);
%             [h3,pv3]= ttest((ratio_mat(3,:)),1);
%             disp(sprintf('COMP TO CHANCE%s %s usw:% f %f bsw: %f %f, mw: %f %f ', ylab, tstr,pv1, h1, pv2, h2, pv3,h3));
%         else 
%             
%         end 
%         
...
%     figure(); 
%     
%     S1_s2p_frac_touch= [0.058 0.108 0.115 0.060 0.037 0.029 0.063 0.071 0.051]; 
%     S1_s2p_frac_NT= [0.022 0.073 0.073 0.035 0.039 0.034 0.033 0.052 0.026]; 
%     S2_s1p_frac_touch= [0.053 0.060 0.084 0.081 0.123 0.077 0.108 0.082 0.102];
%     S2_s1p_frac_NT=[0.013 0.034 0.059 0.031 0.042 0.054 0.095 0.073 0.057];
%     
%     bar (1, mean(S1_s2p_frac_touch)); hold on;
%     plot (1, S1_s2p_frac_touch, 'ko');
%     bar (2, mean(S1_s2p_frac_NT));
%     plot (2, S1_s2p_frac_NT, 'ko');
%     bar (3, mean(S2_s1p_frac_touch));
%     plot (3, S2_s1p_frac_touch, 'ko'); 
%     bar(4, mean(S2_s1p_frac_NT));
%     plot (4, S2_s1p_frac_NT,'ko');
%     ylim([0 0.2]);