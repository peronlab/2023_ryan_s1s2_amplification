function plot_3d_two_whiskers(all_dat, ani_str, di, ax, cell_type, touch, area_str, show_rejects, sat_val, size_range)
...
%             ct_vali = all_dat.types_by_idx{ai}.usw_by_day{di};
    ...
    xyz = nan*zeros(length(w_score),3);
    xyz(:,1) = all_dat.anim_data{ai}.x_um'; 
    xyz(:,2) = all_dat.anim_data{ai}.y_um'; 
    ...
    if (show_rejects)
         ...
         plot3(xyz(invali,1),xyz(invali,2),xyz(invali,3),'.','Color',  [1 1 1]*0.75, 'MarkerSize', 0.2);
     end
