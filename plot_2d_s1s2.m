function plot_2d_two_whiskers(all_dat, ani_str, di, ax, cell_type, touch, area_vali, show_rejects, sat_val, size_range)
    ...
    %if (nargin < 7 || isempty(area)) ; area_str = 's1_all' ;end
...
%             ct_vali = all_dat.types_by_idx{ai}.usw_by_day{di};
    ...
    vali = area_vali; %intersect(area_vali, ct_vali);
    ...
    if (show_rejects)
         ...
         plot(xy(invali,1),xy(invali,2),'.','Color',  [1 1 1]*0.75, 'MarkerSize', 0.2);
     end
    ...
    for v=1:length(vali)
        ...
        w_color = sat_color; %cmap(max(1,round(w_score(i)*100)),:);
        ...
        msize(i) = round(max(size_range(1), (sqrt(w_score(i)))*size_range(2)));
         ...
         if w_score(i)>0.15
             p=plot(xy(i,1),xy(i,2),'o','Color',col,'MarkerFaceColor',col,'MarkerSize',msize(i));
             %alpha (0.5)
         end 
%         
%         if (show_rejects)
...
%         end 
    ...
    ax_w = 750;
    ax_h = 750%300; %500; %change depending on if 2 or 1 subvolumes imaged
    ...
    if (labels_shown)
        ...
        ax.Title.String = sprintf('n=%d cells, %s', length(area_vali), ani_str);
    else
