function stim_corr_type= corrs_allcellsametouch(retro, proj_only,sub_better,discard_an)   
%% setup 
%retro=1; 
%proj_only=1; 
    ...
    for ai=1:length(ani_used)
        ...
        %start by finding MW Cells in one animal. day 1
         ...
         for c=1:length(cts) 
                ...
                for tt=1:4 % loop through 4 touch types: w1p, w1r, w2p, w2r. trying to find s1, mw cells that specifically respond to each touch type 
                    ...
                    num_cells (c, tt, ai)= length(idi); %sanity check if we need cell # cutoffs
                    ...
                    mean_corr_ancurr (c,tt)= mean(cti_corrs_per_cell);
                end 
         end
         ...
         %mean_plot(:,dd,ai)= nanmean(means,2); %columns are animals, rows are cell type: usw/bsw/mw   
            ...
            %mean_plot(:,dd,ai)= nanmean(means,2); %columns are animals, rows are cell type: usw/bsw/mw
            mean_plot_rlly(:,ai)= nanmean(mean_corr_ancurr,2);
     ...
     for ctns=1:3 
         ...
         plot (ax, x1(ctns),(nonzeros(mean_plot_rlly(ctns,:))),'ko')
         ax.TickDir='out';ax.Box='off';
     
     end  
       string_title= sprintf('all cells that touch type (stim locked).imaging in: %s, Retro: %i, Projection: %i, usw/bsw/mw n= %i/%i/%i', area_ofi, retro,proj_only, n(1),n(2),n(3))
